...
public class BeeHiveLoreModule implements PaperExtrasModule, Listener {

  private final String beeHiveLoreBees, beeHiveLoreHoney;
  private final MiniMessage miniMessage = PaperExtras.getInstance().miniMessage;
  ...
  protected BeeHiveLoreModule() {
    ...
    this.beeHiveLoreHoney =
        config.getString(
            "settings.items.beehive-lore.honey", "<reset><gray>Honey level: <honey>/<maxhoney>");
  }
  ...
  public boolean shouldEnable() {
    return PaperExtras.getPluginConfig().getBoolean("settings.items.beehive-lore.enabled", false);
  }
  ...
  public void onBeehiveDrop(BlockDropItemEvent event) {
    ...
    org.bukkit.block.data.type.Beehive beehiveData =
        (org.bukkit.block.data.type.Beehive) blockState.getBlockData();
    ...
    for (Item itemDrop : itemDrops) {
      ...
      if (item.getType().equals(Material.BEE_NEST) || item.getType().equals(Material.BEEHIVE)) {
        ...
        Component beeCountComponent =
            getBeesComponent(beehive.getEntityCount(), beehive.getMaxEntities());
  ...
  private Component getHoneyComponent(int honey, int maxHoney) {
    ...
    beeHiveLoreHoneyString =
        beeHiveLoreHoneyString.replaceAll("<maxhoney>", String.valueOf(maxHoney));
