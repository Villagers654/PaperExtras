...
public class ShieldSettingsModule implements PaperExtrasModule, Listener {
    ...
    public boolean shouldEnable() {
        shieldDamageReduction = PaperExtras.getPluginConfig().getDouble("settings.shield.damage-reduction", shieldDamageReduction);
        shieldDamageReduction = Math.min(1.0, Math.max(0, shieldDamageReduction));

        shieldCooldown = PaperExtras.getPluginConfig().getInt("settings.shield.cooldown", shieldCooldown);
