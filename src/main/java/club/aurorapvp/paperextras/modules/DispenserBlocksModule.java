 ...
 *
 * <p>**break-blocks** If a tool category is enabled, that tool dispensed from dispenser will
 * destroy the block in front of it. It will only destroy blocks that tool can destroy and it will
 * destroy them like that tool was used on it, so wooden pickaxe will destroy diamond ore, but will
 * not drop any items.
 *
 * <p>**shears-shear-pumpkin** If enabled, when shears are dispensed and there's a pumpkin in front
 * of a dispenser, shears will be used, making carved pumpkin.
 *
 * <p>**interact-with-cauldron** If enabled, will allow dispensers fill and empty cauldrons.
 *
 * <p>**put-discs-in-jukebox** If enabled, dispensers will be able to insert into or swap music
 * discs in jukeboxes.
 */
public class DispenserBlocksModule implements PaperExtrasModule, Listener {

  private static final MaterialSetTag CAULDRON_BUCKETS =
      new MaterialSetTag(new NamespacedKey(PaperExtras.getInstance(), "cauldron_buckets"))
          .add(Material.BUCKET)
          .add(Material.WATER_BUCKET)
          .add(Material.LAVA_BUCKET)
          .add(Material.POWDER_SNOW_BUCKET);

  BlockData waterCauldron =
  ...
  public void onPreDispense(BlockPreDispenseEvent event) {
    ...
    if (interactWithCauldron
        && Tag.CAULDRONS.isTagged(block.getType())
        && CAULDRON_BUCKETS.isTagged(item)) {
      ...
      switch (block.getType()) {
        case LAVA_CAULDRON -> {
          fullCauldronHandler(
              block, item, blockDispenser, Material.LAVA_BUCKET, Sound.ITEM_BUCKET_FILL_LAVA);
          event.setCancelled(true);
        }
        case WATER_CAULDRON -> {
          ...
          fullCauldronHandler(
              block, item, blockDispenser, Material.WATER_BUCKET, Sound.ITEM_BUCKET_FILL);
          event.setCancelled(true);
        }
  ...
  private boolean tryBreakBlock(
      ItemStack itemStack, org.bukkit.block.Dispenser dispenser, Block block) {
  ...
  private void emptyCauldronHandler(
    ...
    dispLocation
        .getWorld()
        .playSound(dispLocation, Sound.BLOCK_DISPENSER_DISPENSE, SoundCategory.BLOCKS, 1, 1);
  ...
  private void fullCauldronHandler(
    ...
    dispenserBlock
        .getWorld()
        .playSound(dispenserBlock.getLocation(), uniqueSound, SoundCategory.BLOCKS, 1, 1);
  }
}
