...
public class FurnaceBurnTimeModule implements PaperExtrasModule, Listener {
    ...
    protected FurnaceBurnTimeModule() {
        furnaceBurnTimeMultiplier = PaperExtras.getPluginConfig().getDouble("settings.furnace.burn-time.multiplier", 1.0);
    }
    ...
    public boolean shouldEnable() {
        if (!PaperExtras.getPluginConfig().getBoolean("settings.furnace.burn-time.enabled", false)) return false;
