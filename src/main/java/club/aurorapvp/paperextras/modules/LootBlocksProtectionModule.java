...
public class LootBlocksProtectionModule implements PaperExtrasModule, Listener {
  ...
  protected LootBlocksProtectionModule() {
    ...
    allowBreakingInSneak =
        config.getBoolean("settings.protect-blocks-with-loot.allow-breaking-in-sneak", true);
    try {
      messageType =
          MessageType.valueOf(
              config
                  .getString("settings.protect-blocks-with-loot.message-type", "CHAT")
                  .toUpperCase());
  ...
  public boolean shouldEnable() {
    return PaperExtras.getPluginConfig()
        .getBoolean("settings.protect-blocks-with-loot.enabled", false);
  }
