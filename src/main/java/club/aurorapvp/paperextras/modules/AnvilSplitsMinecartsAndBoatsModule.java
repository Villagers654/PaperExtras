...
/**
 * If enabled, dropping an anvil from significant height onto minecart with content (chest minecart,
 * furnace minecart, etc.) in its item form, it will not destroy the item, but split the minecart
 * and the content and drop them both. Does the same to chest boats
 */
public class AnvilSplitsMinecartsAndBoatsModule implements PaperExtrasModule, Listener {

  private final boolean splitBoats, splitMinecarts;

  protected AnvilSplitsMinecartsAndBoatsModule() {
    this.splitMinecarts =
        PaperExtras.getPluginConfig().getBoolean("settings.anvil-splits-minecarts", false);
    this.splitBoats =
        PaperExtras.getPluginConfig().getBoolean("settings.anvil-splits-boats", false);
  }
  ...
  public boolean shouldEnable() {
    return PaperExtras.getPluginConfig().getBoolean("settings.anvil-splits-minecarts", false)
        || PaperExtras.getPluginConfig().getBoolean("settings.anvil-splits-boats", false);
  }
