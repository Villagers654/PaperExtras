 ...
 * This overrides vanilla transformations. Vanilla mob ids are used to identify mobs. There are also
 * special cases:
 *
 * <p>`killer_bunny` - a killer bunny `jeb_sheep` - rainbow sheep `johhny` - vindicator aggressive
 * to most mobs `toast` - special variant of rabbit
 */
public class LightningTransformsMobsModule implements PaperExtrasModule, Listener {
  ...
  protected LightningTransformsMobsModule() {
    ...
    ConfigurationSection section =
        PaperExtras.getPluginConfig()
            .getConfigSection("settings.lightning-transforms-entities.entities", defaults);
  ...
  public boolean shouldEnable() {
    ...
    return PaperExtras.getPluginConfig()
        .getBoolean("settings.lightning-transforms-entities.enabled", false);
  }
  ...
  private void spawnEntity(Object entity, Location location) {
    if (entity instanceof EntityType entityType) {
      location
          .getWorld()
          .spawnEntity(location, entityType, CreatureSpawnEvent.SpawnReason.LIGHTNING);
