...
public class CreeperSquidsModule implements PaperExtrasModule, Listener {
    ...
    public void enable() {
        ...
        maxDistance = (int) Math.pow(PaperExtras.getPluginConfig().getInt("settings.creeper-squid.agro-distance", 7), 2);
        explosionPower = PaperExtras.getPluginConfig().getInt("settings.creeper-squid.explosion-radius", 3);
    ...
    private final class SquidGoal implements Goal<Squid> {
        ...
        private static final GoalKey<Squid> goalKey = GoalKey.of(Squid.class, PaperExtras.key("squidgoal"));
