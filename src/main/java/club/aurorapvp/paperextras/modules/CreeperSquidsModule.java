...
/**
 * Allows squids to act as Creepers - they will explode if within {@link #maxDistance} after a fuse
 * of {@link #maxSwell} ticks, as well as moving towards you at a velocity of {@link #velocity}
 * Explosion power / radius is also configurable through {@link #explosionPower}
 */
public class CreeperSquidsModule implements PaperExtrasModule, Listener {
  ...
  @Override
  public void enable() {
    Bukkit.getServer().getPluginManager().registerEvents(this, PaperExtras.getInstance());
    maxSwell = PaperExtras.getPluginConfig().getInt("settings.creeper-squid.fuse-ticks", 30);
    maxDistance =
        (int)
            Math.pow(
                PaperExtras.getPluginConfig().getInt("settings.creeper-squid.agro-distance", 7), 2);
    explosionPower =
        PaperExtras.getPluginConfig().getInt("settings.creeper-squid.explosion-radius", 3);
    velocity = PaperExtras.getPluginConfig().getDouble("settings.creeper-squid.velocity", 0.5);
  }

  @Override
  public boolean shouldEnable() {
    return PaperExtras.getPluginConfig().getBoolean("settings.creeper-squid.enabled", false);
  }
  ...
  private final class SquidGoal implements Goal<Squid> {
    ...
    private static final GoalKey<Squid> goalKey =
        GoalKey.of(Squid.class, PaperExtras.key("squidgoal"));
    private static final Predicate<Player> playerPredicate =
        player ->
      ...
      if (currentTarget != null) {
        ...
        squid.setVelocity(
            squid
                .getLocation()
                .toVector()
                .subtract(currentTarget.getLocation().toVector())
                .normalize()
                .multiply(-1 * velocity));
        ...
        Bukkit.getScheduler()
            .runTaskLater(
                ...
                () ->
                    squid
                        .getWorld()
                        .playSound(squid.getLocation(), Sound.ENTITY_CREEPER_DEATH, 1f, 0.5f),
                1);
      } else {
        this.currentSwell = 0;
      }
      ...
      Collection<Player> players =
          squid
              ...
              .getNearbyEntitiesByType(
                  Player.class, squid.getLocation(), maxDistance, playerPredicate);
